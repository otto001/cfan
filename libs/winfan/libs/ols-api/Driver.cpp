...
BOOL ManageDriver(LPCTSTR DriverId, LPCTSTR DriverPath, USHORT Function)
{
    ...
    if(hSCManager == NULL)
	{
		std::cout << "HSCManager open failed";
        return FALSE;
    }
...
BOOL InstallDriver(SC_HANDLE hSCManager, LPCTSTR DriverId, LPCTSTR DriverPath)
{
	...
	{
		error = GetLastError();
		std::cout << "INSTALL DRIVER FAILED: " << error << "\n";
	...
	if (rCode) {
		std::cout << "INSTALL DRIVER SUCCESS \n";
	}
...
BOOL StartDriver(SC_HANDLE hSCManager, LPCTSTR DriverId)
{
    ...
    if(hService != NULL)
	{
		if(! StartService(hService, 0, NULL))
		{
			...
			error = GetLastError();
			std::cout << "START DRIVER FAILED " << error << " \n";
		...
		std::cout << "START DRIVER COULD NOT OPEN DRIVER SERVICE \n";

	}
	if (rCode) {
		std::cout << "START DRIVER SUCCESS \n";
	}
	return rCode;
}
