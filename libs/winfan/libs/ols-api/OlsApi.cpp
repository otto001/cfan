...
#include <intrin.h>
#include "OlsDll.h"
#include "include/ols-api/OlsApi.h"
#include "OlsDef.h"
#include "Driver.h"
#include "OlsIoctl.h"
...
VOID WINAPI WriteIoPortByte(WORD port, BYTE value)
{
	...
	inBuf.data.CharData = value;
	inBuf.PortNumber = port;
	length = offsetof(OLS_WRITE_IO_PORT_INPUT, data.CharData) +
						 sizeof(inBuf.data.CharData);
...
VOID WINAPI WriteIoPortWord(WORD port, WORD value)
{
	...
	inBuf.data.ShortData = value;
	inBuf.PortNumber = port;
	length = offsetof(OLS_WRITE_IO_PORT_INPUT, data.CharData) +
						 sizeof(inBuf.data.ShortData);
...
VOID WINAPI WriteIoPortDword(WORD port, DWORD value)
{
	...
	inBuf.data.LongData = value;
	inBuf.PortNumber = port;
	length = offsetof(OLS_WRITE_IO_PORT_INPUT, data.CharData) +
						 sizeof(inBuf.data.LongData);
